...
function UniteCreatorParamsDialog(){
	...
	function initImageSelectField() {
		var objImageWrapper = g_objWrapper.find(".unite-setting-image");

		if (objImageWrapper.length === 0)
			return;

		var objSettings = new UniteSettingsUC();

		objImageWrapper.each(function () {
			objSettings.initImageChooser(jQuery(this));
		});
